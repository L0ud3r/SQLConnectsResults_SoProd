...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		var isLocked = view.isEventLocked(event);
...
TimeGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		var event = seg.event;
		var isLocked = view.isEventLocked(event);
		var isDraggable = !isLocked && view.isEventDraggable(event);
		var isResizableFromStart = !isLocked && !disableResizing && seg.isStart && view.isEventResizableFromStart(event);
		var isResizableFromEnd = !isLocked && !disableResizing && seg.isEnd && view.isEventResizableFromEnd(event);
		var classes = this.getSegClasses(seg, isDraggable, isResizableFromStart || isResizableFromEnd);
		var skinCss = cssToStr(this.getEventSkinCss(event));
		var timeText;
		...
		return '<a class="' + classes.join(' ') + (isLocked ? ' tooltips" title="' + htmlEscape(fullTimeText) + '"' :'"') +
			(event.url ?
				' href="' + htmlEscape(event.url) + '"' :
				''
				) +
			...
			(timeText ? 
						'<div class="fc-time"' +
						' data-start="' + htmlEscape(fullTimeText) + '"' +
						' data-full="' + htmlEscape(fullTimeText) + '"' +
						'>' +
			'<span>' + (!isLocked?htmlEscape(timeText):'') + '</span>' +
			'</div>' :
						''
						) +
					(event.title ?
...
var View = fc.View = Class.extend({
	...
	// Computes if the given event is locked
	isEventLocked: function (event) {
		var source = event.source || {};

		return firstDefined(
			event.locked,
			source.locked,
			this.opt('eventStartLocked'),
			event.locked,
			source.locked,
			this.opt('locked')
		);
